...
transformed parameters {
  ...
  for (a in 1:N){
    for (t in 1:T){
      for (i in 1:R){
        mmd[R*(T*(a-1)+t-1)+i] = cons + A_a[a] + R_a[i] + RA[i,a] + (t == 1 ? 0 : A_b[a]*k[t-1]) + (t == 1 ? 0 : k2[t-1]*R_b[i]);// + ((a > N-2 && t == 1)) || (a == N && t == 2) ? 0 : coh[N+t-2-a]; 
      }
    }
  }
...
model {
  ...
  k[1] ~ normal(0.0, sigk[1]);
  k[2:(T-1)] ~ normal(slope[1]*k[1:(T-2)], sigk[1]);
...
generated quantities {
  ...
  kf[1] = normal_rng(slope[1]*k[T-1],sigk[1]);  
  for (t in 2:F) {
    kf[t] = normal_rng(slope[1]*kf[t-1], sigk[1]);
  }
  k2f[1] = normal_rng(slope[2] * k2[T-1],sigk[2]);
  for (t in 2:F) {
    k2f[t] = normal_rng(slope[2] * k2f[t-1],sigk[2]);
  }
  for (a in 1:N){
    for (t in 1:F){
      for (i in 1:R){
        mmd_f[R*(F*(a-1)+t-1)+i] = cons + A_a[a] + R_a[i] + RA[i,a] + A_b[a]*kf[t]+ k2f[t]*R_b[i];// + coh[N+T+t-2-a];
      }
    }
  }
}
