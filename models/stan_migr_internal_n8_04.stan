...
generated quantities {
  ...
  kf2[1] = normal_rng(intercept[2] + slope[2]*k2[T-1], sigk[2]); //intercept[2] + 
  for (t in 2:F) kf1[t] = normal_rng(intercept[1] + slope[1]*kf1[t-1], sigk[1]);//intercept[1] +
  for (t in 2:F) kf2[t] = normal_rng(intercept[2] + slope[2]*kf2[t-1], sigk[2]); //intercept[2] + 
  for (a in 1:N){
    for (s in 1:S){
      for (t in 1:F){
        for (i in 1:Co){
