...
transformed parameters {
  ...
  for (a in 1:N){
    for (s in 1:S){
      for (t in 1:T){
        for (i in 1:R){
          mmd[R*(T*(S*(a-1)+s-1)+t-1)+i] = cons + G*(s-1) + RA[i,a] + A_a[a,s] + (t == 1 ? 0 : A_b[a,s]*k[t-1,s]) + (t<23 ? 0 : def[i])+R_a[i] + (t == 1 ? 0 : k2[t-1]*R_b[i]);
        }
      }
    }
  }
...
generated quantities {
  ...
  for (a in 1:N){
    for (s in 1:S){
      for (t in 1:F){
        for (i in 1:R){
          mmd_f[R*(F*(S*(a-1)+s-1)+t-1)+i] = cons + G*(s-1) + RA[i,a] + A_a[a,s] +  A_b[a,s]*kf[t,s] + def[i]+ R_a[i] +k2f[t]*R_b[i];
        }
      }
    }
  }
