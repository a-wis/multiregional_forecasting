data {
  ...
  // S+AS+OA+DA
  //AR k restricted to positive through exp() NOT WORKING
...
transformed parameters {
  ...
  for (a in 1:N){
    for (s in 1:S){
      for (t in 1:T){
        for (i in 1:Co){
          mmd[Co*(T*(S*(a-1)+s-1)+t-1)+i] = G*(s-1) +  SA[a]*(s-1) + OD_a[i] + cOA[i,a] + cDA[i,a]+ A_a[a] + (t == 1 ? 0 : A_b[a]*k1[t-1]) + (t == 1 ? 0 : OD_b[i]*k2[t-1]);
        }
      }
    }
  }
...
model {
  ...
  // k1[2:(T-1)] ~ normal(slope[1] * k1[1:(T-2)], sigk[1]);
  // k2[1] ~ normal( 0.0, sigk[2]);
  // k2[2:(T-1)] ~ normal(slope[2] * k2[1:(T-2)], sigk[2]);
...
generated quantities {
  ...
  for (a in 1:N){
    for (s in 1:S){
      for (t in 1:F){
        for (i in 1:Co){
          mmd_f[Co*(F*(S*(a-1)+s-1)+t-1)+i] = G*(s-1) + SA[a]*(s-1) + OD_a[i] + cOA[i,a] + cDA[i,a]+A_a[a] + A_b[a]*kf1[t] + OD_b[i]*kf2[t];
        }
      }
    }
  }
  ...
  // for (i in 1:N) for (t in 1:T-1) k1b[t,i] = A_b[]
}
