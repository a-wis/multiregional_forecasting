data {
  ...
  //
    ...
    //t priors rather than normal for sig2 in 004; def is random, ar no drift
    // NOT CONVERGING
...
model {
  ...
  k[1] ~ normal(0.0, sigk);
  k[2:(T-1)] ~ normal(slope*k[1:(T-2)], sigk);
...
generated quantities {
  ...
  kf[1] = normal_rng(slope*k[T-1],sigk);  
  for (t in 2:F) {
    kf[t] = normal_rng(slope*kf[t-1], sigk);
  }
