...
model {
  ...
  k[1] ~ normal(intercept, 1);
  k[2:(T-1)] ~ normal(intercept + slope*k[1:(T-2)], 1);
  k2[1] ~ normal(intercept2, 1);
  k2[2:(T-1)] ~ normal(intercept2 + slope2 * k2[1:(T-2)], 1);
  //Model
...
generated quantities {
  ...
  kf[1] = normal_rng(intercept + slope*k[T-1],1);  
  for (t in 2:F) {
    kf[t] = normal_rng(intercept + slope*kf[t-1], 1);
  }
  k2f[1] = normal_rng(intercept2 + slope2 * k2[T-1],1);
  for (t in 2:F) {
    k2f[t] = normal_rng(intercept2 + slope2 * k2f[t-1],1);
  }
  for (a in 1:N){
    for (s in 1:S){
      for (t in 1:F){
