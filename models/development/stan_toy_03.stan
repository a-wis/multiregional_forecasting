...
transformed parameters {
  ...
  for (r in 1:(R-1)) R_a[r] = R_arnd[r] * sig2[2];
  ...
  k1[1] = k1_0;// + k1_rnd[1]*sigk[1];
  k2[1] = k2_0;// + k2_rnd[1]*sigk[2];
  for (i in 2:(T)) {
    k1[i] = k1[i-1] + k1_rnd[i-1]*sigk[1];
    k2[i] = k2[i-1] + k2_rnd[i-1]*sigk[2];
  }
  
  for (a in 1:N){
    for (t in 1:T){
      for (i in 1:R){
        // mmd[R*(T*(a-1)+t-1)+i] = (i == 1 ? 0 : R_a[i-1]) + A_a[a] + (t == 1 ? 0 : A_b[a]*k1[t-1]) + (t == 1 ? 0 : R_b[i]*k2[t-1]);
        mmd[R*(T*(a-1)+t-1)+i] = (i == 1 ? 0 : R_a[i-1]) + A_a[a] + A_b[a]*k1[t] + R_b[i]*k2[t];
      }
    }
  }
...
model {
  ...
  ben1 ~ multi_normal_prec(A_bp[1:(N-1)],mat_R1);
  ...
  ben2 ~ multi_normal_prec(R_bp[1:(R-1)],mat_R2);
