...
transformed parameters {
  ...
  for (r in 1:Co) OD_a[r,1] = OD_uni[unicorr[r]]  + sig3[3]*odau_snd[r];
  for (t in 2:T){
    for (r in 1:Co) OD_a[r,t] = OD_uni[unicorr[r]] + OD_a[r,t-1] + sig3[3]*odau_snd[r];
  }
  ...
  // 
  // ODT[i,1] = k1_rnd[1]*sigk[1];
  
  
  k1[1] = k1_rnd[1]*sigk[1];//k1_0; + ;
  // k2[1] = k2_rnd[1]*sigk[2];// + ;k2_0;
  for (i in 2:(T-1)) {
    k1[i] = k1[i-1] + k1_rnd[i]*sigk[1];
    // k2[i] = k2[i-1] + k2_rnd[i]*sigk[2];
  }
  ...
  for (a in 1:N){
    for (s in 1:S){
      for (t in 1:T){
        for (i in 1:Co){
          mmd[Co*(T*(S*(a-1)+s-1)+t-1)+i] = G*(s-1) +  SA[a]*(s-1) + OD_a[i,t] + cOA[i,a] + cDA[i,a]+ A_a[a] + (t == 1 ? 0 : A_b[a]*k1[t-1]); //+ (t == 1 ? 0 : OD_b[i]*k2[t-1]);
        }
      }
    }
  }
...
generated quantities {
  ...
  // kf2[1] = normal_rng( k2[T-1], sigk[2]); //intercept[2] + 
  for (t in 2:F) kf1[t] = normal_rng( kf1[t-1], sigk[1]);//intercept[1] +
  // for (t in 2:F) kf2[t] = normal_rng(kf2[t-1], sigk[2]); //intercept[2] + 
  for (a in 1:N){
    for (s in 1:S){
      for (t in 1:F){
        for (i in 1:Co){
          mmd_f[Co*(F*(S*(a-1)+s-1)+t-1)+i] = G*(s-1) + SA[a]*(s-1) + OD_af[i,t] + cOA[i,a] + cDA[i,a]+A_a[a] + A_b[a]*kf1[t];// + OD_b[i]*kf2[t];
        }
      }
    }
  }
