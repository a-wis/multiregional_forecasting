...
transformed parameters {
  ...
  for (i in 1:(N-1)) A_a[i] = A_arnd[i] * sig2[1];
  ...
  for (a in 1:N){
    for (t in 1:T){
      for (i in 1:R){
        mmd[R*(T*(a-1)+t-1)+i] = cons + (i == 1 ? 0 : R_a[i-1]) + (a == 1 ? 0 : A_a[a-1]) + (t == 1 ? 0 : A_b[a]*k1[t-1]) + (t == 1 ? 0 : R_b[i]*k2[t-1]);
...
model {
  ...
  k2[1] ~ normal( intercept, sigk[2]);
  k2[2:(T-1)] ~ normal(intercept+slope[2]*k2[1:(T-2)], sigk[2]);
