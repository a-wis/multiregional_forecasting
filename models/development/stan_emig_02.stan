...
transformed parameters {
  ...
  for (a in 1:N){
    for (s in 1:S){
      for (t in 1:T){
        for (i in 1:R){
          mmd[R*(T*(S*(a-1)+s-1)+t-1)+i] = cons + RA[i,a] + A_a[a,s] + (t == 1 ? 0 : A_b[a]*k[t-1]) + (t<23 ? 0 : def[i])+R_a[i] + (t == 1 ? 0 : k2[t-1]*R_b[i]);
        }
      }
    }
  }
...
model {
  ...
  for (a in 1:N){
    for (s in 1:S){
      A_a[a,s] ~ normal(0,sig2[1]);
    }
  }
  for (a in 1:R){
    R_a[a] ~ normal(0,sig2[2]);
  }
  def ~ normal(0,sig3[2]);
  ...
  k[1] ~ normal(intercept, sigk);
  k[2:(T-1)] ~ normal(intercept + slope*k[1:(T-2)], sigk);
  k2[1] ~ normal(intercept2, sigk2);
  k2[2:(T-1)] ~ normal(intercept2 + slope2 * k2[1:(T-2)], sigk2);
  //Model
...
generated quantities {
  ...
  kf[1] = normal_rng(intercept + slope*k[T-1],sigk);  
  for (t in 2:F) {
    kf[t] = normal_rng(intercept + slope*kf[t-1], sigk);
  }
  k2f[1] = normal_rng(intercept2 + slope2 * k2[T-1],sigk2);
  for (t in 2:F) {
    k2f[t] = normal_rng(intercept2 + slope2 * k2f[t-1],sigk2);
  }
  for (a in 1:N){
    for (s in 1:S){
      for (t in 1:F){
        for (i in 1:R){
          mmd_f[R*(F*(S*(a-1)+s-1)+t-1)+i] = cons + RA[i,a] + A_a[a,s] +  A_b[a]*kf[t] + def[i]+ R_a[i] +k2f[t]*R_b[i];
        }
      }
    }
  }
