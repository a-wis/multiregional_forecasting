...
transformed parameters {
  ...
  for (i in 1:T) k1[i] = kn1[i] - mean(kn1[1:T]);
  for (i in 1:T) k2[i] = kn2[i] - mean(kn2[1:T]);

  // for (i in 1:N) A_a[i] = A_arnd[i] * sig2[1];
  // for (r in 1:R) R_a[r] = R_arnd[r] * sig2[2];

  kn1[1] = intercept+k1_rnd[1]*sigk[1];
  kn2[1] = k2_rnd[1]*sigk[2];
  for (i in 2:(T)) {
    kn1[i] = intercept+slope[1]*kn1[i-1] + k1_rnd[i]*sigk[1];
    kn2[i] = slope[2]*kn2[i-1] + k2_rnd[i]*sigk[2];
  }
